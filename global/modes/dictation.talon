...
# Everything here should call auto_insert to preserve
# the state to correctly auto-capitalize/auto-space.
<user.prose>:
    auto_insert(prose)
...
cap <user.word>:
    capitalized_word = user.formatted_text(word, "CAPITALIZE_FIRST_WORD")
    auto_insert(capitalized_word)
...
spell that <user.formatters> <user.letters>:
    formatted_letters = user.formatted_text(letters, formatters)
    user.dictation_insert_raw(formatted_letters)
