...
FILE_EXTENSIONS = csv.read_spoken_forms(
    "file_extensions.csv", value_name="File extension"
)

RE_FILE_EXTENSIONS = "|".join(
    re.escape(file_extension.strip()) + "$"
    for file_extension in FILE_EXTENSIONS.values()
)

RE_CHUNK = re.compile("|".join([RE_WORD, RE_FILE_EXTENSIONS]))
...
ONES = ["", *(k for k, v in DIGITS_MAP.items() if 1 <= v <= 9)]
TEENS = list(TEENS_MAP.keys())
TWENTIES = ["", "", *(k for k, v in TENS_MAP.items() if 20 <= v)]
THOUSANDS = ["", *SCALES[1:]]


def create_spoken_form_for_number(num: int):
    """Creates a spoken form for a number."""
    global ONES, TEENS, TWENTIES, THOUSANDS, SCALES
...
def create_spoken_form_years(num: str):
    ...
    if centuries % 10 != 0:
        ...
        # 200X -> two thousand x
        if remainder < 9:
            words.append(ONES[centuries // 10])
...
ABBREVIATIONS = csv.read_spoken_forms("abbreviations.csv", value_name="Abbreviation")
ALPHABET = csv.read_spoken_forms("alphabet.csv", value_name="Letter")
...
################################################################################
# Spoken forms via regular expressions
################################################################################


def create_spoken_forms_from_regex(source: str, pattern: re.Pattern) -> Sequence[str]:
    """
...
# test_spoken_form("BlockingIOError", ["blocking sit odd error"])
